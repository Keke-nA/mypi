...
原因很简单：`AI` 层先稳定统一协议，`Agent` 层再围绕统一协议建立 turn loop，`TUI` 层消费事件做展示，`Coding-Agent` 层把 session tree、工具和终端交互装配起来，最后再由 `mom` 这一层补齐 Slack、workspace、sandbox、events 和长期运行语义。
...
- [Mom 复现计划](./mom.md)：定义 `mypi` 如何在现有 `AI` / `Agent` / `Coding-Agent` core 之上，补齐 Slack、per-channel workspace、sandbox、attachments、memory、skills 和 events，形成接近 `pi-mono` mom 的上层产品形态。
...
- [Mom 复现计划](./mom.md)
  用于把当前 CLI 型 `coding-agent` 继续扩展为 Slack bot 形态，明确 `mom` 的包布局、与 `coding-agent` 的复用边界、per-channel workspace、sandbox、attachments、memory、skills、events 和分阶段落地顺序。
