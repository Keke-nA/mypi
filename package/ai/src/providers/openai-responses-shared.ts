...
export function convertResponsesMessages<TApi extends Api>(
	...
	for (const msg of transformedMessages) {
		if (msg.role === "user") {
			...
			for (const block of msg.content) {
				if (block.type === "thinking") {
					...
					output.push({
						...
						status: "completed",
						id: msgId,
						phase: parsedSignature?.phase ?? null,
					} satisfies ResponseOutputMessage);
				} else if (block.type === "toolCall") {
					const toolCall = block as ToolCall;
					const [callIdMaybe, itemIdRaw] = toolCall.id.split("|");
					const callId = callIdMaybe ?? toolCall.id;
			...
			const [callIdMaybe] = msg.toolCallId.split("|");
			const callId = callIdMaybe ?? msg.toolCallId;
